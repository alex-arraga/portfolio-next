...
export function Calculator() {
        ...
        if (context) {
            ...
            if (fotmatingScreen !== valueScreen) {
                if ((!/,\d{3,}/.test(valueScreen))) {
                    context.setValueScreen(fotmatingScreen)
                }
            }
        }
    }, [valueScreen, context]);
    ...
    if (context) {
        const saveOperation = context.saveOperation
        const setValueScreen = context.setValueScreen
        const lastResult = context.lastResult
        const setLastResult = context.setLastResult
        ...
        const convertExpression = () => {
            ...
            convertValue = convertValue.replace(expSquare, (_, value: MathExpression) => {
                return Math.sqrt(evaluate(value)).toString();
            });

            convertValue = convertValue.replace(expLog, (_, value: MathExpression) => {
                return Math.log10(evaluate(value)).toString();
            });

            convertValue = convertValue.replace(expLn, (_, value: MathExpression) => {
                return Math.log(evaluate(value)).toString();
            });

            convertValue = convertValue.replace(expTan, (_, value: MathExpression) => {
                return Math.tan(evaluate(value)).toString();
            });

            convertValue = convertValue.replace(expCos, (_, value: MathExpression) => {
                return Math.cos(evaluate(value)).toString();
            });

            convertValue = convertValue.replace(expSin, (_, value: MathExpression) => {
